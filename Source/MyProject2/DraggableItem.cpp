...
FReply UDraggableItem::NativeOnMouseButtonDown(const FGeometry& InGeometry,
...
void UDraggableItem::NativeOnDragDetected(const FGeometry& InGeometry,
	const FPointerEvent& InMouseEvent, UDragDropOperation*& OutOperation)
{
	OutOperation = UWidgetBlueprintLibrary::CreateDragDropOperation(
		UFruitDragDropOperation::StaticClass());
	if (OutOperation)
	{
		UFruitDragDropOperation* FruitDragDropOperation =
			Cast<UFruitDragDropOperation>(OutOperation);
		...
		FruitDragDropOperation->OnDragCancelled.AddDynamic(
			this, &UDraggableItem::OnDragCancelledHandle);
	}
	else
	{
...
bool UDraggableItem::NativeOnDragOver(const FGeometry& InGeometry,
	const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	auto verticalBox = Cast<UVerticalBox>(GetParent());
	UFruitDragDropOperation* FruitDragDropOperation =
		Cast<UFruitDragDropOperation>(InOperation);
	if (FruitDragDropOperation && verticalBox
		&& this != FruitDragDropOperation->DefaultDragVisual)
	{
		int32 IndexDragged = verticalBox->GetChildIndex(
			FruitDragDropOperation->DefaultDragVisual);
		if (IndexDragged != -1)
		{
...
bool UDraggableItem::NativeOnDrop(const FGeometry& InGeometry,
	const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	if (InOperation && InOperation->DefaultDragVisual)
	{
		Cast<UUserWidget>(InOperation->DefaultDragVisual)->SetColorAndOpacity(
